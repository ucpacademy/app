...
export default async function BranchPage({
  ...
  const { data: branch } = await supabase
    .from('branches')
    .select(
      `*, translations:branch_translations(*), major:majors(*, translations:major_translations(*))`,
    )
  ...
  const translation =
    branch.translations?.find((t: { lang: string }) => t.lang === lang) ||
    branch.translations?.[0];
  const majorTranslation =
    branch.major?.translations?.find(
      (t: { lang: string }) => t.lang === lang,
    ) || branch.major?.translations?.[0];
  const faqs = Array.isArray(translation?.faqs) ? translation.faqs : [];
              ...
              <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 md:p-10 shadow-sm border border-slate-200 dark:border-slate-800">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {branch.gallery.map((img: string, idx: number) => (
                    <div
                      key={idx}
                      className="aspect-square rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700"
                    >
                      <img
                        src={img}
